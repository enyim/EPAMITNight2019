...
namespace ITNight
{
	...
	public class SpanCron: NaiveCronExpressionBase
	{
		...
		public static SpanCron Parse(string value)
		{
			...
			return new SpanCron(minute, hour, day, month, week);
		}

		private static NaiveRule ParseRule(ref ReadOnlySpan<char> s, int min, int max)
		{
		...
		private static bool ParseListItem(ref ReadOnlySpan<char> s, int min, int max, HashSet<int> values)
		{
			// ?
			// *[/step]
			// from[-to][/step]
